...
public class Solution {
    ...
    public static void sort(List<Stock> list) {
        Collections.sort(list, new Comparator<Stock>() {
            //Метод возвращающий прибыль в зависимости от использованого конструктора объекта Stock, на фиг не нуженен, как оказалось
            double chang (Stock stock){
                if(stock.containsValue("change")){
                ...
                if(result == 0) {
                    ...
                    if(result == 0){result = Double.compare((((double) stock2.get("last"))- ((double) stock2.get("open"))),(((double) stock1.get("last"))- ((double) stock1.get("open"))));}
                }
                return result;
                /*int result = ((String)stock1.get("name")).compareTo( (String)stock2.get("name"));
                if(result != 0) {
                    ...
                    else {result = Double.compare(chang(stock1),chang(stock2));
                        if (result != 0) {
                            return result;
                        }
